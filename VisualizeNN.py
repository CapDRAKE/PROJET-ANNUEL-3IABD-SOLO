...
class Layer():
    ...
    def __line_between_two_neurons(self, neuron1, neuron2, weight=0.4, textoverlaphandler=None,maxw=1):
    ...
    def draw(self, layerType=0, weights=None,  bias=None,textoverlaphandler=None,maxw=1):
        j=0 # index for neurons in this layer
        for neuron in self.neurons:            
            i=0 # index for neurons in previous layer
            neuron.draw( self.neuron_radius, id= "{:3.2f}".format(bias[j]/maxw)  if bias is not None else  j+1   )
            if self.previous_layer:
                for previous_layer_neuron in self.previous_layer.neurons:
                    self.__line_between_two_neurons(neuron, previous_layer_neuron, weights[i,j], textoverlaphandler,maxw=maxw)
                    i=i+1
            j=j+1
...
class NeuralNetwork():
    ...
    def draw(self, weights_list=None,bias_list=None,fig=None,epoch=0,maxw=1):
        ...
        if fig is None :  fig = pyplot.figure(figsize=(12, 9))
        #else : pyplot.clf()
        for i in range( len(self.layers) ):
            layer = self.layers[i]                                
            if i == 0:
                layer.draw( layerType=0 ,maxw=maxw)
            elif i == len(self.layers)-1:
                layer.draw( layerType=-1, weights=weights_list[i-1],bias =bias_list[i-1] if bias_list is not None else None, textoverlaphandler=overlaphandler,maxw=maxw)
            else:
                layer.draw( layerType=i, weights=weights_list[i-1],bias =bias_list[i-1] if bias_list is not None else None ,textoverlaphandler=overlaphandler,maxw=maxw)

        pyplot.axis('scaled')
        pyplot.axis('off')
        pyplot.title( 'Neural Network epoch :'+ str(epoch) + " weightFactor : "+str(maxw), fontsize=10 )
        figureName='ANN_.png'
...
class DrawNN():
    ...
    def __init__( self, neural_network, weights_list=None,bias_list=None ):
        ...
        for W in self.weights_list:
            for Wx in W:
                for weight in Wx:
                    if  self.maxweight<weight: self.maxweight=weight
                    if  self.maxweight<-weight: self.maxweight=-weight     
    ...
    def draw( self ,fig=None,epoch=0):
        widest_layer = max( self.neural_network )
        network = NeuralNetwork( widest_layer )
        for l in self.neural_network:
            network.add_layer(l)
        return network.draw(self.weights_list,self.bias_list,fig=fig,epoch=epoch,maxw=self.maxweight) 